...
export default function StudyDecksScreen() {
  ...
  const { id, addCount } = useLocalSearchParams<{ id: string; addCount: string }>();
    ...
    <View style={[commonStyles.container, { flex: 1, paddingBottom: 30 }]}>
      ...
      <View style={[styles.buttonBox, { opacity: isSubmitting ? 0.6 : 1 }]}>
        {[
          ...
          <Pressable key={btn.val} onPress={() => handleRate(btn.val)} disabled={isSubmitting}>
            <View style={[btn.color, styles.ratingButton]}>
              <Typography variant="h3" color={colors.darkColor}>{btn.label}</Typography>
            </View>
          </Pressable>
        ))}
      </View>
